...
rf_run = function (n, fft, seed=123) {
  ...
  rf.x = caret::train(x=train.features, y = train.x, method="rf", ntree = trees_num, metric="RMSE")
  
  # save model for X coordinate
  model_x_file = paste0(models_folder, "rf_caret_x_", filename, "_", trees_num, "_", seed, "_", subsample_fraction, ".rds")
  saveRDS(rf.x, file=model_x_file)
  
  print("Y coordinate training ...")
  rf.y = caret::train(x=train.features, y = train.y, method="rf", ntree = trees_num, metric="RMSE")
  
  # save model for Y coordinate
  model_y_file = paste0(models_folder, "rf_caret_y_", filename, "_", trees_num, "_", seed, "_", subsample_fraction, ".rds")
